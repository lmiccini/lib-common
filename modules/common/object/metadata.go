...
//
// Compute guardReady with subcr.RotationGuardReady from lib-common.
...
func ManageRotationGracePeriod(
	...
	if !rotationPending {
		if annotations != nil {
			if _, has := annotations[RotationGraceAnnotation]; has {
				delete(annotations, RotationGraceAnnotation)
	...
	graceUntilStr := ""
	if annotations != nil {
		graceUntilStr = annotations[RotationGraceAnnotation]
	}
	...
	if graceUntilStr == "" {
		...
		annotations[RotationGraceAnnotation] = graceUntil.Format(time.RFC3339)
		obj.SetAnnotations(annotations)
		if err := c.Update(ctx, obj); err != nil {
	...
	if err != nil {
		// Malformed timestamp — clear it and let the next reconcile reset
		delete(annotations, RotationGraceAnnotation)
		obj.SetAnnotations(annotations)
